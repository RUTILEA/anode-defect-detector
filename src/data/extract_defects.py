...
class DefectExtractor:
        ...
        for folder in sorted(os.listdir(self.annotations_root))[:4]:
            ...
            for file, content in data.items():
                ...
                for region in regions.values():
                    ...
                    matched_folder = self._find_matched_folder(battery_id_folder)
                    if not matched_folder:
                        print(f"No matching image folder for battery ID: {battery_id_folder}")
                        continue

                    matched_file = self._find_matched_file(file, matched_folder)
                    if not matched_file:
    ...
    def _find_matched_folder(self, battery_id):
    ...
    def _find_matched_file(self, file_key, folder):
