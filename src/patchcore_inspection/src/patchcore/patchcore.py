...
class PatchCore(torch.nn.Module):
    ...
    def predict_without_ground_truth(self, dataloader):
        """Predict scores and masks without requiring ground truth labels or masks."""
        _ = self.forward_modules.eval()

        scores = []
        masks = []

        with tqdm.tqdm(dataloader, desc="Inferring (no GT)...", leave=False) as data_iterator:
            for batch in data_iterator:
                if isinstance(batch, dict):
                    batch = batch["image"]  # Only use image tensor
